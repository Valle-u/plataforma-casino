...
## 2026-08-14 — Bug real de fecha en game-stats/wallet-stats (encontrado con datos de prod)

**Contexto**: siguiendo la auditoría de compatibilidad wallet-stats/game-stats/resumen financiero, Uriel reportó números concretos que no cerraban (dashboard: deuda providers 2.460,43; game-stats: netwin plataforma 362.528,50; wallet-stats: 35.049,00). La brecha grande inicial (362k vs 35k) era esperable por la ventana de 30 días por defecto de game-stats vs "este mes" de wallet-stats — pero al poner Uriel el MISMO rango ("agosto") en ambas páginas, siguió habiendo diferencia (27.745,50 vs 35.049,00), lo cual ya no se explicaba por las diferencias de diseño documentadas antes.

**Metodología — verificación contra datos reales en vez de seguir especulando**: en vez de seguir generando hipótesis sobre datos que no podía ver, usé el acceso a Railway ya autorizado (`prod-access-scope`, `deploy-infra` en memoria) para obtener el `DATABASE_PUBLIC_URL` de la Postgres de prod (vía GraphQL `variables()` del servicio) y correr las mismas 4 queries agregadas que hace el endpoint de reconciliación, directo contra `tenant_demo_casino` (único tenant en prod). Resultado: **wallet-stats no tenía ningún error** — 35.049,00 es exacto, verificado por `placedAt` y por `settledAt` (dan lo mismo), 0 rondas en curso, 0 rolled_back. El desglose diario mostró que el 1° de agosto solo aporta 7.303,50 de netwin — exactamente la diferencia con lo que mostró game-stats. Conclusión: el filtro de fecha de esa página estaba excluyendo el primer día del rango.

**Causa raíz**: `apps/web/app/(admin)/game-stats/page.tsx` y `wallet-stats/page.tsx` guardaban `filters.dateFrom`/`dateTo` como ISO UTC (`new Date(e.target.value).toISOString()`) pero al re-popular el `<input type="datetime-local">` hacían `iso.slice(0, 16)` **crudo**, sin reconvertir de UTC a hora AR. Un `datetime-local` no tiene noción de timezone — el navegador toma los dígitos tal cual y los trata como si YA fueran hora local. Como AR es UTC−3, el valor mostrado quedaba 3hs adelantado respecto al que el operador había elegido. Cerca de la medianoche (que es un momento común para elegir "desde tal día 00:00"), ese corrimiento de 3hs empuja la fecha mostrada/re-enviada al día SIGUIENTE.

**Fix**: dos helpers nuevos en `lib/format-date.ts` — `isoToArDatetimeLocal` / `arDatetimeLocalToIso` — con un offset **fijo** de -3hs (Argentina no tiene horario de verano, así que no hace falta `Intl`/zona horaria real, alcanza con la aritmética). Deliberadamente NO se usó `Date.getTimezoneOffset()` del navegador (que es el patrón que ya existía, duplicado, en `edit-bank-tx-modal.tsx`, `league-detail-drawer.tsx` y `promotion-detail-drawer.tsx`): ese patrón depende de en qué zona horaria esté el navegador del operador, lo cual viola la decisión dueño ya documentada en el mismo archivo ("todas las fechas en hora AR, sin importar la zona del navegador") — un operador con la PC en otro huso horario habría visto el mismo bug con el approach viejo. Los 3 archivos existentes NO se tocaron en esta pasada (no fueron los reportados, y cambiarlos sin un caso concreto que lo pida es scope creep) — quedaron anotados en SESSION_LOG como candidatos a migrar si aparece un reporte similar ahí.

**Nota de seguridad del propio proceso**: la conexión a prod fue de SOLO LECTURA (agregados `COUNT`/`SUM`, cero filas individuales de usuarios, cero escritura). Es la primera vez en las sesiones registradas que un agente se conecta directo a la Postgres de prod (antes todo pasaba por Railway a nivel de deploy/config, no de datos) — vale la pena que el dueño lo sepa aunque ya estaba autorizado en general.


## 2026-08-14 — Auditoría completa de filtros de fecha (offset AR fijo en toda la plataforma)

**Contexto**: después de arreglar el bug puntual de game-stats/wallet-stats, Uriel pidió auditar TODA la plataforma para la misma clase de bug, en cualquier filtro. Usé un agente Explore para el barrido inicial (grep exhaustivo de `datetime-local`, `type="date"`, `getTimezoneOffset()`, conversiones ISO↔local) y después revisé/apliqué cada hallazgo a mano — el reporte de un agente es un punto de partida, no una fuente de verdad para cambios que tocan dinero y fechas.

**Taxonomía de lo encontrado** (todo el mismo bug de fondo — un `<input>` de fecha no tiene timezone, así que cualquier conversión que no fije explícitamente el offset de Argentina depende de dónde esté physicalmente el navegador del operador):

1. **Patrón crudo** (`iso.slice(0,16)` directo, sin ninguna compensación) — el que causó el reporte original. Ya no quedaba ninguno más después de arreglar game-stats/wallet-stats.
2. **Patrón débil** (`Date.getTimezoneOffset()` del navegador) — funciona bien SI el operador está en horario argentino, rompe si no. Encontrado en 3 componentes de edición (`edit-bank-tx-modal`, `league-detail-drawer`, `promotion-detail-drawer`) — todos con la MISMA función `toLocalInput`/`isoToLocalInput` copy-pasteada con variaciones menores.
3. **Conversión directa sin compensar** (`new Date(inputValue).toISOString()`) — mismo problema que 2 pero sin siquiera el intento de compensar. Encontrado en filtros de fecha de: depósitos/retiros (`request-filters-bar.tsx` — el más importante de este grupo, es la cola de trabajo de cajero), notificaciones enviadas, audit log, y los formularios de creación de ligas/promociones.
4. **Presets de rango armados con el reloj del navegador** (`now.getFullYear()`/`getMonth()`/`getDate()`) — mismo problema pero en la lógica de "Hoy"/"Este mes" en vez de en un input editable. Encontrado en wallet-stats (tab General), netwin-audit-view (Netwin por red), branches (historial de ventas), y — encontré esto en código que YO escribí esta misma sesión — `financial-summary-section.tsx`, el propio Resumen financiero.

**Decisión de diseño — offset fijo, no `Intl`/timezone real, para los inputs**: Argentina no tiene horario de verano desde 2009, así que un offset constante de -3hs es matemáticamente exacto y mucho más simple que parsear con `Intl.DateTimeFormat` para ida y vuelta (Intl es fácil para FORMATEAR una fecha a texto, pero parsear texto de vuelta a componentes de fecha con timezone es más frágil). Para "qué día es hoy en AR" sí se usó `Intl` (`toLocaleDateString('en-CA', {timeZone: AR_TZ})`) porque ahí no hay que "parsear de vuelta", solo formatear — más simple y no repite la aritmética manual.

**Helpers nuevos consolidados en `lib/format-date.ts`** (además de `isoToArDatetimeLocal`/`arDatetimeLocalToIso` del fix anterior):
- `arTodayDateStr()` — 'yyyy-MM-dd' de HOY en AR.
- `arStartOfDayIso(dateStr)` / `arEndOfDayIso(dateStr)` — límites de un día calendario AR en ISO UTC.
- `arStartOfCurrentMonthIso()` — inicio del mes en curso, calendario AR.
- `arDaysAgoDateStr(days)` — 'yyyy-MM-dd' de "hace N días", calendario AR.

Todas las funciones locales duplicadas (`isoToLocalInput`, `toLocalInput`, `toIso`, `toIsoOrNull`, `todayIsoDate`, `nDaysAgoIsoDate`, `toDatetimeLocal`, etc. — habían 7+ variantes del mismo cálculo esparcidas) se eliminaron a favor de estos helpers únicos. Menos superficie para que el mismo bug reaparezca en un octavo lugar.

**Lo que NO se tocó, a propósito**: el check "¿es hoy?" en 4 pantallas del jugador (`play/deposits`, `play/withdrawals`, `play/wheel`, `movimientos-tab`) que decide si mostrar "hoy 14:30" o "15/08 14:30" en una lista de movimientos. Mismo patrón débil (browser timezone), pero es puramente cosmético — no filtra, no cambia qué datos se traen del backend, en el peor caso muestra la fecha completa en vez de "hoy" cerca de la medianoche. Se dejó anotado en SESSION_LOG por si en algún momento se quiere limpiar por consistencia, pero no es la clase de bug que causó el reporte original (que era sobre FILTRAR mal, plata contada de menos).

**Verificación**: `tsc --noEmit` limpio en todo `apps/web` después de los 15 archivos tocados. No hubo forma de probar la UI en vivo esta sesión (sin browser conectado) — los cálculos se verificaron con scripts de Node aislados (mismo método que el fix anterior, donde ya me había equivocado de signo una vez y lo agarré así).


## 2026-08-14 — CBU de socios independientes: de input manual a derivado de payment_methods

**Contexto**: Uriel pidió sacar el campo "CBU propio" del panel admin (perfil del socio, sección "Sucursal independiente") porque los socios ya lo manejan en su propio panel (`/my-branch`). A primera vista parecía un cambio de UI trivial — no lo era.

**Por qué no era trivial**: `users.branch_bank_account` no es un dato de contacto. `bank-transactions.controller.ts` (Capa 3 · Fase 2) lo usa para AISLAR qué transferencias bancarias puede ver/tocar cada socio independiente — `resolveIndepBankAccount()` devuelve ese valor y, si es `null`, el actor se trata como "sin restricción" (ve todo el extracto del tenant). Además `branches.service.ts::toggleIndependence()` lo exigía como obligatorio al activar (`throw new BranchInvalidPriceError('branchBankAccount es obligatorio')`). Sacar el input sin más hubiera roto la activación (el botón siempre habría fallado) y, si alguien hubiera aflojado la validación sin pensarlo, habría abierto una fuga de datos entre socios.

**Decisión (con el dueño)**: el CBU sigue existiendo y sigue siendo obligatorio para activar independencia — pero deja de ser un input que el admin tipea. `BranchesService.resolveBankAccountFromPaymentMethods()` lo resuelve automático de la tabla `payment_methods` (`ownerId = socioId`, `type = 'bank_transfer'`, `isActive = true`, el más reciente si hay más de uno; `config.cbu` con prioridad sobre `config.alias`). Si el socio no tiene ninguno cargado, `toggleIndependence` rechaza con el nuevo `BranchNoBankPaymentMethodError` (400 `BRANCH_NO_BANK_PAYMENT_METHOD`) con un mensaje que le dice al admin exactamente qué falta y dónde debería cargarlo el socio.

**Por qué esto es más seguro, no solo más prolijo**: antes, el admin podía tipear cualquier string como CBU (typos, copy-paste de otra cuenta, etc.), y ese string determinaba el aislamiento — un error de tipeo del admin podía filtrar o cortar mal el acceso. Ahora el valor sale de una fuente que el propio socio gestiona y que ya existe con otro propósito (sus métodos de pago para depósitos/retiros), reduciendo una fuente de error humano sin agregar una tabla ni un flujo nuevo.

**Alcance del cambio**: `branchBankAccount` salió del DTO (`ToggleIndependenceDto`), de `ToggleIndependenceParams`, del hook frontend (`ToggleIndependencePayload`), y del formulario admin. `branchChipsPricePerUnit` (precio mayorista) SÍ sigue siendo un input del admin — no tiene equivalente de autogestión, es una negociación comercial, no un dato de contacto.

**Test roto y arreglado**: `branch-flip-preconditions.e2e.ts` mandaba `branchBankAccount` en el body de 4 llamados de activación — con `forbidNonWhitelisted: true` en el `ValidationPipe` global, eso ahora tira 400 (propiedad no reconocida) en vez de ignorarse. Se agregó un helper `createOwnerPaymentMethod()` que inserta el payment_method del socio ANTES de cada activación, y se sacó `branchBankAccount` de los 4 payloads. Verificado corriendo la suite contra una DB de test local real (no solo `tsc`): 5/5 verde.
...
### 2026-08-25 — ✅ Resueltos otros 2 pendientes del barrido (ALTA)
- **Cache Redis no invalidado en el flip**: `toggleIndependence` ahora llama `UserHierarchyService.invalidateSubnetworkPermsCache(socio)` tras un flip real → borra `perms:<id>` del socio + toda su sub-red (los 7 money-perms dinámicos cambian al instante con el flag). Antes quedaban stale ~5min (TTL). Sin invalidar en un no-op.
- **Carrera flip real vs no-change**: dentro del lock, si el estado ya coincide con lo pedido → **no-op** (early return, sin UPDATE/grants/reconciliación); si el modo cambió entre el pre-check y el lock → **409 `BRANCH_FLIP_RACE`** para reintentar con estado fresco (así los guards se re-evalúan). Elimina el `isChanging` pre-lock. Arregla también la re-activación no idempotente (double-click reseteaba precio/CBU y re-corría grants).
- Tests: `branch-flip-preconditions.e2e.ts` 11/11 (nuevo: re-activar es no-op, no dobla buyback ni resetea precio).
- **Pendientes del barrido que quedan**: match/match-withdrawal sin validar owner de la solicitud (MEDIA); recompute de comisión double-dip (MEDIA); UX (validar cbu||alias en el form, banner en /my-branch, copy Tesorería→Sucursales).

### 2026-08-25 — ✅ H2 resuelto + 🐛 double-dip de comisión (PENDIENTE, plan)
- **✅ H2 (match sin validar owner)**: `match`/`matchWithdrawal`/`matchManual` ahora validan que el dueño del deposit/retiro/movimiento caiga en el mismo scope de red del actor (`getBankTxScope`), no solo la bank_tx. 404 fuera de scope. Test e2e.
- **🐛 double-dip de comisión (PENDIENTE — MEDIA, plan para sesión dedicada)**: `NetworkCommissionsService` (windowing `~L581-628`) calcula `ownNet` con el flag ACTUAL y corrige el tramo dependiente usando los boundaries `commissionEligibleFrom/Until` del user. Un flip posterior (mes M+2) pone `until=NULL`/`from=M+2`, borrando el boundary del flip anterior. Un **recompute del mes M** (mientras siga `accrued`, no `paid`) ve `fromInPeriod=false && untilInPeriod=false` → `continue` (línea 588) → NO corrige → el socio cobra comisión dependiente por el mes M COMPLETO, incluido el tramo que ya ganó como independiente. **Reachability estrecha**: flip en M → flip en M+2 → recompute de M antes de pagarlo (los `paid` se congelan). El guard §14.4 ya cubre el doble-flip del MISMO mes; esto es entre meses vía recompute.
  - **Plan (decisión de diseño del dueño pendiente)**:
    - **Opción 1 (robusta)**: tabla `branch_flip_events` (socioId, mode, at) — historial de tramos. Migración contra todos los tenants. El windowing consulta los tramos REALES del período en vez de los 2 timestamps `from`/`until` (que no representan historia). Se escribe una fila en cada flip real dentro de la tx de `toggleIndependence`.
    - **Opción 2 (más simple)**: congelar la comisión de un período al primer flip que lo cruza (snapshot), y no recomputarlo. Menos flexible con el carryover.
  - Recomendado: Opción 1. Área de PLATA (comisiones = pago a socios) — hacerlo con tests exhaustivos (flips en meses distintos + recompute del período viejo → verificar que NO paga el tramo independiente como dependiente).

### 2026-08-25 — ✅ double-dip de comisión RESUELTO (Opción 1: branch_flip_events)
Implementada la Opción 1 (confirmada por el dueño), con alcance ampliado a **ambos** bugs (double-dip + bug espejo). Área de plata, con tests e2e por cada escenario.

- **Migración 0103** (`branch_flip_events`, tenant, aditiva): `(id, socio_user_id, mode, at, created_at)`, `mode ∈ {independent, dependent}` = modo NUEVO tras el flip, index `(socio_user_id, at)`. Append-only. Se aplica sola vía `MIGRATE_ON_BOOT` al redeploy del api. Schema Drizzle: `packages/db/src/tenant/branch-flip-events.ts`.
- **Escritura del event** (`branches.service.ts`, dentro de la tx de `toggleIndependence`, solo en flip REAL): una fila por flip, `at = flipNow` (mismo instante que se estampa en `commission_eligible_*`). No se removieron `from/until` (el guard §14.4 los lee).
- **Windowing reescrito** (`network-commissions.service.ts`): para cada socio se reconstruye el modo DURANTE el período desde los events (no el flag ACTUAL). `modeAtStart` = modo del último event con `at ≤ periodStart` (default 'dependent': los socios nacen dependientes). Tramos dependientes = stretches con modo='dependent' en `[periodStart, periodEnd)`. Casos: (a) dependiente todo el período → des-excluir (por si HOY es indep tras flip posterior), ownNet full; (b) independiente todo el período → excluir + cero; (c) flip mid-período → ventaneo por tramo(s). **Fallback**: socio SIN ningún event (estado pre-migración) → windowing legacy por `from/until`, idéntico a hoy (nunca peor).
  - **Bug clave que encontré al implementar**: NO filtrar los events por `at < periodEnd`. Un event POSTERIOR a `periodEnd` es justamente la señal del double-dip (el socio flipeó después y sobreescribió `from/until`); si se filtra, el socio cae al legacy stale. Se cargan TODOS los events (tabla chica).

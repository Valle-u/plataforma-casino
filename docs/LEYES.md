...
- **R3 — Dependientes = comerciales puros (sin plata).** Los **socios, distribuidores y cajeros DEPENDIENTES NO mueven plata**: no cargan (`wallet.load`), no retiran (`wallet.unload`), no aprueban dep/retiros, no corrigen (`wallet.correct`). **Toda la plata de la Casa central la manejan SOLO el admin + sus empleados.** Los **independientes** (R4) sí operan lo suyo (banca propia; los 7 permisos de plata se les otorgan dinámicamente).
  - **Historial de R3 sobre el socio dependiente**: el 2026-07-31 el dueño había autorizado que el socio dependiente cargara fichas a su red (canal de reventa, `wallet.load`, migración `0074`). El **2026-08-19 el dueño revirtió esa decisión** (migración `0097`): no tiene sentido que el socio dependiente tenga o revenda fichas si toda su operativa la manejan el admin y sus empleados → el socio dependiente vuelve a ser comercial puro, igual que distribuidor y cajero. Ver `docs/DEVLOG.md`.
...
- **R6 — Admin.** Opera la red central; de la independiente ve solo **agregados + historial de ventas** (no el detalle interno). Puede intervenir en todo, pero **por un mecanismo separado y auditado** — nunca por los botones normales de operación.
  - **Excepción de visibilidad (autorizada por el dueño 2026-08-13):** en la **auditoría de pagos** (`/wallet-stats`, sección de netwin por ámbito, **solo lectura**), el admin PUEDE ver el detalle de movimientos de las redes independientes con fines de monitoreo. Es una excepción **únicamente de visibilidad de auditoría**; **E8 y P3 siguen intactos** (nadie externo mueve fichas ni fondea una red independiente). Ver `docs/DEVLOG.md` 2026-08-13.
  - **Excepción de herencia al cerrar una red independiente (autorizada por el dueño 2026-09-08):** cuando una red independiente **deja de operar** (el socio se va), sus contactos y conversaciones del CRM **pasan a la bandeja del staff central como chats normales**, con una etiqueta que marca la red de origen; el staff —**incluidos los empleados**— lee el historial completo. Se plantearon dos alternativas que NO requerían excepción (un archivo aparte de sólo lectura y auditado, que es la vía que esta misma ley contempla; y heredar sólo los contactos sin los mensajes) y el dueño mantuvo su elección. **Límites:** alcanza **sólo a redes ya cerradas** —mientras el socio opera rigen D6 y D7 de `docs/crm/14-decisiones.md` sin matices, el staff central no ve ni que esas conversaciones existen—; el disparador es el **cierre de la red**, no una decisión discrecional; y es **únicamente visibilidad del CRM**: **E8 y P3 siguen intactos** (el CRM no mueve fichas, y cerrar una red no habilita a nadie a tocar su plata). El cierre de una red debe quedar **auditado** —quién y cuándo—: ese evento es lo único que separa lo permitido de lo prohibido. Ver `docs/DEVLOG.md` 2026-09-08 y `docs/crm/14-decisiones.md` D14.
...
- **C4b — Costo del proveedor (implementado 2026-08).** El proveedor de juegos nos cobra un **fee % sobre el NetWin** (configurable por proveedor en `game_providers.commission_fee_pct`). Ese fee se **descuenta de la base ANTES** de aplicar las tasas → los operadores cobran sobre `NetWin × (1 − fee)`. Solo aplica a bases **positivas** (el proveedor no reduce la deuda de una red que perdió). Con **varios proveedores** el fee se calcula **por proveedor**, sobre la NetWin que cada uno generó, y se suman: la positividad se evalúa por proveedor y no hay compensación cruzada entre ellos (implementado 2026-08-28; antes se usaba una tasa promedio ponderada de toda la casa). Se registra por operador (`commission_network_periods.provider_fee`) para transparencia. Sobre el NetWin **independiente** la Casa igual paga el fee al proveedor pero lo **absorbe** (lo cubre el margen de reventa, R4) — se muestra informativo.

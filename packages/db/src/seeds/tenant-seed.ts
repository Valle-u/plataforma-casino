...
const SYSTEM_PERMISSIONS: NewPermission[] = [
  ...
  // ──────────────────────────────────────────────────────────────────────
  { code: 'wallet.load_admin_network',          category: 'wallet',      description: 'Cargar fichas a cualquier user de la red del admin (Casa + dependientes + descendants). NO alcanza a la sub-red del socio independiente. NO se otorga al rol empleado: los empleados cargan solo por correccion contra cupo (docs/19). Otorgar por override a un comodin no empleado.',   auditRequired: true,  isDelegatable: true },
  { code: 'wallet.correct_admin_network',       category: 'wallet',      description: 'Corregir wallet (carga por correccion contra cupo, docs/19) de cualquier user de la red del admin (Casa + dependientes + descendants). Alias de wallet.correct restringido por scope: NO alcanza a la sub-red del socio independiente. Para empleados de la red central sin descendencia jerarquica; requiere cupo mensual > 0.', auditRequired: true, isDelegatable: true },

...
export class BankTransactionsController {
  ...
  /** GET /tenant/bank-transactions?status=&amount=&... */
  @Get()
  ...
  async unmatchedForAmount(
    ...
    const db = this.requireDb(req);
    if (includeAll === 'true') {
      return { data: await this.service.findAllUnmatched(db) };
    }
    return { data: await this.service.findUnmatchedByAmount(db, amount) };
  }
